...
exports.taoYeuCauMuon = async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      return res.status(400).json({ success: false, message: 'Bạn đã mượn sách này rồi' });
    }
...
exports.duyetMuonSach = async (req, res) => {
  ...
  try {
    ...
    if (!muonSach) {
      return res.status(404).json({ success: false, message: 'Không tìm thấy yêu cầu mượn sách' });
    }

    if (trangThai === 'Đã duyệt') {
      // Giảm số lượng sách
      if (muonSach.MASACH.SOQUYEN <= 0) {
        return res.status(400).json({ success: false, message: 'Sách đã hết, không thể duyệt' });
      }
...
exports.xacNhanTraSach = async (req, res) => {
  ...
  try {
    ...
    if (muonSach.TRANGTHAI !== 'Đã duyệt') {
      return res.status(400).json({ success: false, message: 'Chỉ có thể trả sách đã được duyệt' });
    }
