...
router.post('/', auth, role.checkRole(['quantri']), nhaXuatBanController.createNhaXuatBan);
...
// @access  Private (Chỉ quản lý)
router.put('/:id', auth, role.checkRole(['quantri']), nhaXuatBanController.updateNhaXuatBan);

// @route   DELETE api/nhaxuatban/:id
// @desc    Xóa nhà xuất bản
// @access  Private (Chỉ quản lý)
router.delete('/:id', auth, role.checkRole(['quantri']), nhaXuatBanController.deleteNhaXuatBan);
