...
router.post('/', auth, role.checkRole(['docgia']), muonSachController.taoYeuCauMuon);

// @route   PUT api/muonsach/:id/duyet
// @desc    Duyệt yêu cầu mượn sách
// @access  Private (Nhân viên)
router.put('/:id/duyet', auth, role.checkRole(['nhanvien', 'quantri']), muonSachController.duyetMuonSach);

// @route   PUT api/muonsach/:id/tra
// @desc    Xác nhận trả sách
// @access  Private (Nhân viên)
router.put('/:id/tra', auth, role.checkRole(['nhanvien', 'quantri']), muonSachController.xacNhanTraSach);

// @route   GET api/muonsach
// @desc    Lấy tất cả yêu cầu mượn sách
// @access  Private (Nhân viên/Quản lý)
router.get('/', auth, role.checkRole(['nhanvien', 'quantri']), muonSachController.getAllMuonSach);

// @route   GET api/muonsach/docgia/:id
// @desc    Lấy yêu cầu mượn sách của đọc giả
// @access  Private (Đọc giả hoặc nhân viên/quản lý)
router.get('/docgia/:id', auth, (req, res, next) => {
