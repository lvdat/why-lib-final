          ...
          <table class="min-w-full bg-white rounded-lg overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="borrowing in borrowings" :key="borrowing._id">
                <td class="py-3 px-4">
                  <router-link
                    :to="{ name: 'book-detail', params: { id: borrowing.MASACH._id } }"
                ...
                <td class="py-3 px-4">
                  {{ borrowing.NGAYTRA ? new Date(borrowing.NGAYTRA).toLocaleDateString() : 'Chưa trả' }}
                </td>
                <td class="py-3 px-4">
                  <span :class="{
                    'bg-yellow-100 text-yellow-800': borrowing.TRANGTHAI === 'Chờ duyệt',
                    'bg-green-100 text-green-800': borrowing.TRANGTHAI === 'Đã duyệt',
                    'bg-red-100 text-red-800': borrowing.TRANGTHAI === 'Từ chối',
                    'bg-blue-100 text-blue-800': borrowing.TRANGTHAI === 'Đã trả',
